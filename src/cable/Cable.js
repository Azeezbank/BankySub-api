...
router.post('/subscription', async (req, res) => {
    ...
    try {
        ...
        const ApiDocs = await prisma.env.findFirst({ where: { service_type: providerId }, select: { api_key: true, api_url: true } });
...
router.post('/verify/iuc', async (req, res) => {
    ...
    try {
        ...
        if (!apiDocs) {
            console.log('No cable provider found');
            return res.status(404).json({message: 'No cable provider found'})
        }

        const apiKey = decrypt(apiDocs.api_key);
        ...
        const userIdentity = await axios.get(`https://ncwallet.ng/api/cable/cable-validation?cable_id=${providerId}&cable_number=${number}`, { headers: header }
        );
